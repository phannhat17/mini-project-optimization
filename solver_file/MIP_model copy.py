...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        # coordinate
        w[i] = solver.IntVar(0, max_W,'l[%i]' % i)
        r[i] = solver.IntVar(0, max_W,'r[%i]' % i)
        t[i] = solver.IntVar(0, max_H,'t[%i]' % i)
        h[i] = solver.IntVar(0, max_H,'b[%i]' % i)        

        solver.Add(w[i] == (1-Ro[i]) * data['size_item'][i][0] + Ro[i] * data['size_item'][i][1])
        solver.Add(h[i] == (1-Ro[i]) * data['size_item'][i][1] + Ro[i] * data['size_item'][i][0])
        ...
        for m in range(k):
            ...
            solver.Add(r[i] >= (1-x[(i,m)]) * M + w[i])
            solver.Add(t[i] >= (1-x[(i,m)]) * M + h[i])    
    ...
    for i in range(n - 1):
        for j in range(i + 1, n):
            for m in range(k):
                ...
                solver.Add(r[i] <= (r[j]-w[j]) + M * (1 - c1))
                solver.Add(r[j] <= (r[i]-w[i]) + M * (1 - c2))
                solver.Add(t[i] <= (t[j]-h[j]) + M * (1 - c3))
                solver.Add(t[j] <= (t[i]-h[i]) + M * (1 - c4))
